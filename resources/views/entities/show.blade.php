<x-app-layout>
    ...
    <article class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div class="relative h-56 bg-gradient-to-br from-indigo-500 via-violet-500 to-fuchsia-500 sm:h-72">
            @if ($entity->image_url)
                <img src="{{ $entity->image_url }}" alt="{{ $entity->name }}" class="h-full w-full object-cover">
            ...
            <div class="mt-8 rounded-2xl border border-dashed border-indigo-200 bg-indigo-50 p-8 text-center">
                ...
                <div class="mt-8">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-slate-500">
                                Atributos personalizados de esta entidad.
                            </p>
                        </div>
                    ...
                    <div class="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        @forelse ($entity->entityAttributes as $entityAttribute)
                            <article class="rounded-2xl border border-slate-200 bg-slate-50 p-5">
                                ...
                                <div class="mt-3 flex flex-wrap gap-2">
                                    @forelse ($entityAttribute->values as $value)
                                        <span
                                            class="rounded-lg bg-white px-3 py-2 text-sm font-semibold text-slate-800 shadow-sm">
                                            @if ($value->option?->color)
                                                <span class="mr-1 inline-block h-3 w-3 rounded-full"
                                                    style="background-color: {{ $value->option->color }}"></span>
                                            @endif

                                            {{ $value->displayValue() }}

                                            @if ($entityAttribute->attribute->unit)
                                                {{ $entityAttribute->attribute->unit }}
                                            @endif
                                        </span>
                                    @empty
                        ...
                        @empty
                            <div
                                class="sm:col-span-2 lg:col-span-3 rounded-2xl border border-dashed border-slate-300 p-10 text-center">
                                <p class="text-slate-500">
                                    Esta entidad todavía no tiene atributos.
                                </p>
                            </div>
                        @endforelse
                    </div>
