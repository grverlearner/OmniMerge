<x-tournament-layout>
    ...
    <div x-data="groupStageSimulator({
        initializeUrl: @js(route('tournaments.group-stage.simulator.initialize', $phaseTemplate)),
        actionUrl: @js(route('tournaments.group-stage.simulator.action', $phaseTemplate)),
        storageKey: @js('omnimerge:gs-simulator:' . auth()->id() . ':' . $phaseTemplate->id),
        ...
        <section
            class="relative overflow-hidden rounded-[32px] bg-gradient-to-br from-slate-950 via-indigo-950 to-violet-950 p-6 text-white shadow-xl sm:p-8">
            <div class="pointer-events-none absolute -right-24 -top-24 h-72 w-72 rounded-full bg-indigo-400/15 blur-3xl">
            </div>
            ...
            <div class="relative flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
                <div class="max-w-3xl">
                    ...
                    <p class="mt-3 max-w-2xl text-sm leading-7 text-indigo-100/80">
                        Genera participantes ficticios y ejecuta la configuración real
                        ({{ $settings->group_count_mode_label }}, {{ $settings->distribution_mode_label }}) —
                        grupos, calendario interno, reglas de clasificación y puertas de
                        salida — exactamente como se ejecutaría en un torneo real. Nada de
                        esto se guarda: es completamente temporal y aislado.
                    </p>
                </div>
