<x-tournament-layout>
    ...
    <div x-data="roundRobinSimulator({
        initializeUrl: @js(route('tournaments.round-robin.simulator.initialize', $phaseTemplate)),
        actionUrl: @js(route('tournaments.round-robin.simulator.action', $phaseTemplate)),
        storageKey: @js('omnimerge:rr-simulator:' . auth()->id() . ':' . $phaseTemplate->id),
        ...
        <section
            class="relative overflow-hidden rounded-[32px] bg-gradient-to-br from-slate-950 via-cyan-950 to-emerald-950 p-6 text-white shadow-xl sm:p-8">
            ...
            <div class="relative flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
                <div class="max-w-3xl">
                    ...
                    <p class="mt-3 max-w-2xl text-sm leading-7 text-cyan-100/80">
                        Genera participantes ficticios y ejecuta la configuración real
                        ({{ $settings->cycles_label }}) — calendario, puntuación, empates y
                        puertas de salida — exactamente como se ejecutaría en un torneo real.
                        Nada de esto se guarda: es completamente temporal y aislado.
                    </p>
                </div>
