@php
    /*
     * Una rama del árbol de valores de un catálogo.
     *
     ...
     *
     * Solo se dibujan los valores activos —los que vienen en `$todos`—, porque
     * un valor archivado no se puede elegir y colgar ramas de él confundiría.
     */
...
<div style="margin-left: {{ $sangria }}px">
    ...
    <div class="flex items-center gap-2 rounded-xl border border-slate-800 bg-slate-950 p-1.5"
        ...
        <a href="{{ route('attribute-options.show', $nodo) }}"
        ...
        <a href="{{ route('attribute-options.show', $nodo) }}"
            class="min-w-0 flex-1 truncate text-[11px] font-black text-white">
            {{ $nodo->name }}
        </a>
        ...
        <span class="shrink-0 rounded-lg border px-1.5 py-0.5 font-mono text-[9px] font-black"
            style="border-color: {{ $color }}40; color: {{ $nodo->values_count > 0 ? $color : '#475569' }}"
